...
public class LessonController {
	...
	public String review_list(Model model, @RequestParam @Nullable String bid) {
		Map<String, Object> map = lessonService.reviewListAll(bid);

		map = lessonService.reviewListAll(bid);
		model.addAttribute("map", map);
		return "redirect:/lessonBoard/lessonBoard02";
	}

	// 리뷰 DB업로드
	@RequestMapping(value = "/review_write", method = RequestMethod.POST)
	@ResponseBody
	public String review_write(ReviewDTO reviewDto, @RequestParam @Nullable String search, Model model) {
		
		Map<String, Object> map = new HashMap<String, Object>();
		lessonService.reviewWrite(reviewDto);
